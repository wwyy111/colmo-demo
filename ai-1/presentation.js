...
export function createPresentation(onStage,onStop,onTick=()=>{}){
 ...
 function resume(){if(running)return;if(index<0||completed){start();return;}running=true;anchor=performance.now();const token=++revision;onStage(shots[index],offset);onTick(shots[index],offset/shots[index].duration,offset);if(running&&token===revision)raf=requestAnimationFrame(tick);}
