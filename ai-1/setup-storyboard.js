export const momentNames=['表达需求','推荐方案','确认体验','反馈与确认'];
...
export function setupSequence(manifest={},repeat=true,branch='normal'){
 const voice=(id,min=0)=>Math.max(min,(manifest.voices?.[id]?.duration||6500)+650);
 ...
 if(branch==='lightFailure')return [shot('checkLight',2,'检查灯光响应',{view:'setupLight',card:'checking',state:'processing',duration:2500}),shot('lightFailure',2,'灯光未响应',{view:'setupLight',card:'lightFailure',voice:'lightFailure',state:'speaking',effect:'exception',duration:voice('lightFailure')}),shot('failureWait',2,'等待处理',{card:'lightFailure',view:'setupLight'})];
 if(branch==='enableFailure')return [shot('saving',3,'检查并保存',{card:'saving',state:'processing',duration:2500}),shot('enableFailure',3,'启用未成功',{card:'enableFailure',state:'speaking',voice:'enableFailure',effect:'exception',duration:voice('enableFailure')})];
 ...
 if(repeat)shots.push(shot('retryUser',3,'选择再次体验',{card:'adjusted',voice:'retryUser',role:'user',bubble:true,state:'listening',duration:voice('retryUser')}),shot('retryIntro',2,'按新方案再次体验',{card:'retry',voice:'retry',state:'speaking',duration:voice('retry')}),shot('retryLight',2,'更柔和、更缓慢的灯光',{view:'setupLight',card:'retry',state:'executing',duration:8500,light:[.06,.19]}),shot('retryCurtain',2,'保持原定窗帘安排',{view:'setupCurtain',card:'retry',state:'executing',duration:5500,light:[.19,.19],curtain:[.03,.20]}),shot('retryEnd',3,'再次体验结束',{card:'adjusted',duration:2000,light:[.19,.06],curtain:[.20,.03]}));
